...
import { Heart, Zap, Target, AlertTriangle, Lock } from "lucide-react";
...
export default function DiscoveredProfileInfo() {
  ...
  return (
    <div className="space-y-12">
      {/* Visual hacker: apenas dados de Tinder, nada pessoal */}
      <div className="relative mx-auto max-w-3xl rounded-2xl neon-frame-hacker bg-gradient-to-br from-[#071a09]/80 via-[#141a24]/70 to-[#0d0315]/90 border-2 border-green-400/20 shadow-xl p-8 md:p-12 overflow-hidden select-none">
          ...
          {images.map((img) => (
            ...
            >
              <img
                ...
                alt="Foto borrada do perfil de Tinder"
        ...
        {/* Só dados de perfil do Tinder */}
        <div className="grid md:grid-cols-2 gap-8">
          <div className="grid gap-5 text-left text-green-100 text-lg md:text-xl font-mono">
            <div className="flex items-center gap-3">
              <Zap className="w-7 h-7 text-yellow-300" />
              <span>
                <span className="font-bold text-yellow-200">Último acesso:</span> há 1 semana
              </span>
            </div>
            <div className="flex items-center gap-3">
              <Target className="w-7 h-7 text-pink-400" />
              <span>
                <span className="font-bold text-pink-200">Estimativa de matches:</span> 7+
              </span>
            </div>
            <div className="flex items-center gap-3">
              <Heart className="w-7 h-7 text-red-400" />
              <span>
                <span className="font-bold text-red-200">Conversas recentes:</span> 3+
              </span>
            </div>
            <div className="flex items-center gap-3">
              <Lock className="w-7 h-7 text-green-400" />
              <span>
                <span className="font-bold text-green-300">Tipo de perfil:</span> Privado
              </span>
            </div>
            <div className="flex items-center gap-3">
              <span className="inline-block w-5 h-5 rounded-full border-2 border-green-400 bg-green-700 mr-2"/>
              <span>
                <span className="font-bold text-green-300">Status do perfil:</span> Ativo
              </span>
            </div>
          </div>
          <div className="md:pl-10 space-y-8">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge className="border border-yellow-400 bg-yellow-900/50 text-yellow-100 px-4 py-2 text-base font-bold">
                🕵️ Atividade recente detectada
              </Badge>
            </div>
            <div className="flex flex-col gap-1 pt-6 text-xs text-gray-400 opacity-90 border-t border-green-800/50">
              <span>❗ Mostramos apenas dados do Tinder. Nenhuma informação pessoal é exibida.</span>
              <span>🔍 As imagens acima são apenas ilustrativas (filtro de privacidade aplicado).</span>
            </div>
          </div>
        </div>
      </div>
